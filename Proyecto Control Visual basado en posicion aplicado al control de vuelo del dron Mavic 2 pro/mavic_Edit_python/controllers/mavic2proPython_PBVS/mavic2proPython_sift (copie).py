...
targetX, targetY, target_altitude = 0.0, 0.5, 0.0
...
startSim = True
while (robot.step(timestep) != -1 and startSim):
    time = robot.getTime()
    dt = time - last_time
    last_time = time
    ...
    if startControl:
        ...
        if len(good)>MIN_MATCH_COUNT:
            ...
            vels_x[j] = vel_lin[0,0]
            vels_y[j] = -vel_lin[2,0]
            vels_z[j] = vel_lin[1,0]
            vels_yaw[j] = vel_ang[1,0]
            
            
            if it % dim == 0:
                j = 0
                
                Tc = np.array([[xGPS], [yGPS], [zGPS], [yaw]])
